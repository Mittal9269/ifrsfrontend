...
import PerBlogComment from './PerBlogComment';
...
export default function PerBlog() {
    ...
    useEffect(() => {
        if (cookies.jwttoken) {
            ...
            fetch(process.env.REACT_APP_BACKEND + "/blog/comment/" + id , {
                method: "GET",
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json'
    ...
    const changeLike = () => {
        if (selfLike) return;
        if (like === true) {
            fetch(process.env.REACT_APP_BACKEND + "/blog/blog/unlike/" + id, {
                method: "PATCH",
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json'
            ...
            fetch(process.env.REACT_APP_BACKEND + "/blog/blog/like/" + id, {
                method: "PATCH",
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json'
    ...
    const SubmitComment = (e) => {
        ...
        fetch(process.env.REACT_APP_BACKEND + "/blog/blog/comment/" + id, {
            method: "POST",
            headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json'
                            ...
                            <div class="wrapper wrapper--w960">
                                <div class="card card-2">
                                    <div class="row px-4 py-1 my-4 text-center">
                                        <div class="col-sm-3">
                                            <img src={data.userID.image} class="img-rounded" alt=" " />
                                            <div class="review-block-name"><p>{data.userID.Name}</p></div>
